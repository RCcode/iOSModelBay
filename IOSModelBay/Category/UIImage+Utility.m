...
- (UIImage *)setMaxResolution:(CGFloat)maxResolution imageOri:(UIImageOrientation)ori {
    ...
    UIImage *temp = [originalImage fixOrientation:ori];
    //防止坐标系混乱
    UIImage *scaleImage = [temp resize:CGSizeMake(newWidth, newHeight)];
    NSLog(@"newWidth = %f   newHeight = %f",originalImage.size.width,originalImage.size.height);
